...
function Show-Menu {
    ...
    Write-Host "  [5]  Solo: Crear Share de Perfiles Moviles" -ForegroundColor Gray
    ...
    Write-Host "  [8b] Solo: Crear GPO de Perfiles Moviles" -ForegroundColor Gray
...
do {
    ...
    switch ($opcion) {
        ...
        "5"  {
            Write-Host "`n>> Creando Share de Perfiles Moviles..." -ForegroundColor Cyan
            New-SharePerfiles
        }
        "6"  {
            Write-Host "`n>> Creando usuarios desde CSV (con ProfilePath)..." -ForegroundColor Cyan
            Import-Module ActiveDirectory
        ...
        "8b" {
            Write-Host "`n>> Creando GPO de Perfiles Moviles..." -ForegroundColor Cyan
            Import-Module GroupPolicy
            New-GPOPerfilesMóviles
        }
        "9"  {
            Write-Host "`n>> Configurando cuotas FSRM sobre carpetas de perfil..." -ForegroundColor Cyan
            Import-Module FileServerResourceManager
            $usuarios = Import-Csv $Global:CsvPath
            New-CuotasFSRM -Usuarios $usuarios
        }
        "10" {
            Write-Host "`n>> Configurando File Screening sobre carpetas de perfil..." -ForegroundColor Cyan
            Import-Module FileServerResourceManager
            $usuarios = Import-Csv $Global:CsvPath
